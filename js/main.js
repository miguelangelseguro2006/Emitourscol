...
/*
  imgSrc(destino, jsonImagen)
  ─────────────────────────────────────────────────────────────
  Prioridad:
    1. Campo "imagen" del JSON  →  lo que subió el cliente en el CMS
    2. Fallback automático      →  assets/img/<clave>.jpg
       donde <clave> = nombre sin tildes, sin espacios, minúscula

  Ejemplos:
    "/assets/img/sanandres.jpg"   (viene del CMS)
    "assets/img/cancun.jpg"       (fallback)
*/
...
async function initHeroSlider() {
  ...
  /*
    Si hay datos JSON: usa los destinos marcados heroSlide:true
    Si no hay datos (sin conexión): usa fallback de colores
  */
  const heroDestinos = data
    ? data.filter(d => d.activo !== false && d.heroSlide === true)
    : [];
...
async function initTestimonios() {
  ...
  /* Soporte para ambos formatos:
     - Array directo:       [ {...}, {...} ]
     - Objeto con items:    { "items": [ {...} ] }   (cuando el CMS guarda así)
  */
